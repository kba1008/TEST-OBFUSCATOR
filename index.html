...
<head>
  ...
  <script>
    ...
    let gpsWatchId = null; // ID Tracker GPS berterusan
      ...
      if (isInZone) {
        ...
        if (!session && locked && lockedKat && !isSubmitting && !isAlreadyActiveDB) {
          if (inZoneStart > 0) {
            ...
            if(statusEl && !statusEl.innerHTML.includes('SEMAK & AKTIFKAN')) statusEl.innerHTML += ` <br>[AUTO-PUNCH: ${remainingSec}s]`;
    ...
    function handleGPSError(err) {
      ...
      if (!sessionStorage.getItem('gps_warned_interval')) {
         ...
         // Set timeout to allow popup again after 15 seconds if it still fails
  ...
  <style>
    ...
    #statusText { font-size: 11px; font-weight: 600; margin-bottom: 15px; color: #95a5a6; background: #f8f9fa; padding: 4px 12px; border-radius: 15px; display: inline-flex; align-items: center; gap: 5px; }
...
<body oncontextmenu="return false;"> 
  ...
  <div class="container">
    ...
    <div class="content-padding">
      ...
      <button id="btnRetryGPS" class="btn btn-yellow" onclick="retryGPS()" style="display:none; font-size:12px; padding:10px; margin-bottom:15px; margin-top:-5px; font-weight:bold;">
...
<script>
  ...
  function showModernAlert(title, msg, type, callback) {
    ...
    if(type === 'error') {
      ...
      mIcon.innerText = "📍"; mTitle.style.color = "#c0392b";
      window.tempCallback = callback;
      mBtns.innerHTML = `<button class="modal-btn btn-ok" onclick="confirmAction()" style="background:#f39c12; color:white; font-size:14px;">↻ SEMAK GPS SEKARANG</button>`;
    } else {
